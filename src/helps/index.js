...
module.exports = (urlProvider = 'http://localhost:8545') => {
    ...
    let getABIandByteCodefromContract = (contractName, cb) => {
        ...
        let file_link = path.join(__dirname, '../../contracts/xdayteam.sol');
    ...
    let addNewAccount = (obj, from, to, method, cb) => {
        getContractInstance('XDAYTEAM', to, (err, instance) => {
            ...
            var rawTx = {
                ...
                to: to,
                value: '0x0',
                data: instance[method].getData(obj.idAddress, obj.name, obj.phone, obj.role)
            };
    ...
    /**idAddress, idProduct, name, price */
    let addNewProduct = (obj, from, to, method, cb) => {
        getContractInstance('XDAYTEAM', to, (err, instance) => {
            var Tx = require('ethereumjs-tx');
            var privateKey = new Buffer(from.private, 'hex')
            ...
            var rawTx = {
                ...
                to: to,
                value: '0x0',
                //0x${idProduct} convert ObjectID to 16Byte
                data: instance[method].getData(obj.idAddress, obj.idProduct, obj.name, obj.price)
            };
    ...
    /**
     * @{params} obj idAddress, balance 
     */
    let addBalance = (obj, from, to, method, cb) => {
        getContractInstance('XDAYTEAM', to, (err, instance) => {
            var Tx = require('ethereumjs-tx');
            var privateKey = new Buffer(from.private, 'hex')
            ...
            var rawTx = {
                ...
                value: '0x0',
                //0x${idProduct} convert ObjectID to 16Byte
                data: instance[method].getData(obj.idAddress, obj.balance)
            };
    ...
    let addTransaction = (obj, from, to, method, cb) => {
        console.log({
            obj
        })
        getContractInstance('XDAYTEAM', to, (err, instance) => {
            ...
            var rawTx = {
                ...
                value: '0x0',
                //0x${idProduct} convert ObjectID to 16Byte
                data: instance[method].getData(obj.accountA, obj.accountB, obj.idProduct, obj.transaction, obj.priceShip, obj.addressCustomer)
            };
